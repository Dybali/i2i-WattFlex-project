...
public class ApiExceptionHandler {
    ...
    ResponseEntity<ErrorBody> invalid(MethodArgumentNotValidException e) {
        var m = e.getBindingResult().getFieldErrors().stream()
                .map(x -> x.getField() + ": " + x.getDefaultMessage())
                .findFirst()
                .orElse("Geçersiz istek");
        return ResponseEntity.badRequest().body(new ErrorBody(m, Instant.now()));
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ErrorBody> general(Exception e) {
        e.printStackTrace(); // <--- GİZLENEN HATAYI KONSOLA DÖKECEK OLAN SATIR
